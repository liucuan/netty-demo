 ...
 */
public class WebSocketConvertHandler extends MessageToMessageCodec<WebSocketFrame, WebSocketConvertHandler.WebSocketFrame> {//编码 WebSocketFrame 消息转为 WebSocketFrame 消息
    ...
    protected void encode(ChannelHandlerContext ctx, WebSocketFrame msg, List<Object> out) throws Exception {
        ...
        switch (msg.getType()) {//检测 WebSocketFrame 的 FrameType 类型，并且创建一个新的响应的 FrameType 类型的 WebSocketFrame
            case BINARY:
    ...
    protected void decode(ChannelHandlerContext ctx, io.netty.handler.codec.http.websocketx.WebSocketFrame msg, List<Object> out) throws Exception {
        if (msg instanceof BinaryWebSocketFrame) {//通过 instanceof 来检测正确的 FrameType
    ...
    public static final class WebSocketFrame {//自定义消息类型 WebSocketFrame

        public enum FrameType {//枚举类明确了 WebSocketFrame 的类型
